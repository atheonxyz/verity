...
- `VERITY_SWIFT_SDK_MODE=native` is reserved for environments where `output/Verity.xcframework` has already been built and linked in an Apple-native workflow. Use Xcode’s iOS simulator test runner for native verification.
...
- `core/build/build-android.sh` builds `arm64-v8a` by default and accepts `VERITY_ANDROID_ABIS=arm64-v8a,x86_64` to produce additional ABI outputs.
